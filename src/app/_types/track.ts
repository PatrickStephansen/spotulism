  ...
  images?.reduce(
    (current, largest) =>
      ...
      (largest?.height ?? 0) * (largest?.width ?? 0)
        ? current
        : largest,
    { height: 0, width: 0, url: "default-album.png" }
  );

export const trackOrEpisodeToPlayableTrack = (item: any) => ({
  uri: item.uri,
  name: item.name,
  durationMs: item.duration_ms,
  imageUrl: getLargestImage(item.images).url,
  creators: item.artists?.map((a: Artist) => ({ name: a.name, uri: a.uri })) ?? [
